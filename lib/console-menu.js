...
export class ConsoleMenu {
	constructor(options = {}, configPath = './menu-config.json') {
		...
		if (options &&
			...
			if (this.shouldValidate) {
				this.validateMenuStructureSync();
			}
		} else {
	...
	displayMenu() {
		...
		menuItems.forEach((item, index) => {
			...
			let tmp = String(index + 1).padStart(2, '0');
			ColorConsole.print(`${theme.getColor('itemBracket')}[${theme.getColor('itemNumber')}${tmp}${theme.getColor('itemBracket')}]: ${theme.getColor('itemText')}${name}`);
		});

		ColorConsole.print(`\n${theme.getColor('navHint')}[${theme.getColor('navHighlight')}B ${theme.getColor('navText')}to Go Back, ${theme.getColor('navHighlight')}Q ${theme.getColor('navText')}to quit${theme.getColor('navHint')}]`);
	...
	validateMenuStructureSync() {
		...
		// Report duplicate variables (just log warnings, don't wait for user input)
		if (warnings.length > 0 && !this.isSuppressWarnings) {
			console.log("⚠️  Menu validation warnings:");
			warnings.forEach(warning => console.log(`  - ${warning}`));
		}
