...
export default class ColorConsole extends EventEmitter {
	...
	static getColoredStr(str) {
		// HACK: Quick ANSI converter for prompts—retro Wildcat without the print drama!
		// TODO: Maybe add more escape codes for blink/underline if we channel full ANSI art vibes.
	...
	static setPromptHandler(handler) {
		ColorConsole._promptHandler = typeof handler === 'function' ? handler : null;
	}
	...
	print(str, options = {}) {
		...
		// Set return just to be safe
		if (!Object.keys(options).includes("return")) {
			options["return"] = true
		}

		const coloredStr = ColorConsole.getColoredStr(str);  // Reuse for consistency!
		...
		if (coloredStr.includes('${spinner}')) {
			...
			if (options.return === false) {
				...
				const handler = ColorConsole._promptHandler;
				if (typeof handler === 'function') {
					handler(`${coloredStr}\x1b[0;0m`);
				} else {
					process.stdout.write(`${coloredStr}\x1b[0;0m`);
	...
	stopSpinner() {
		if (this.#spinnerInterval) {
			...
			process.stdout.write('\r\x1b[K\n');  // Wipe lingering line AND drop to new one—prompt paradise!
