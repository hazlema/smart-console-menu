...
class CLIManager extends EventEmitter {
	...
	createInterface() {
		if (!this.rl) {
			...
			ColorConsole.setPromptHandler((prompt) => {
				// The prompt is already processed by ColorConsole.getColoredStr()
				// Just write it directly to avoid double-processing
	...
	questionColored(str) {
		...
		return this.question(ColorConsole.getColoredStr(str));
	}
