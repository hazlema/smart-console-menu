	...
	async promptForVariable(varName) {
		...
		if (options.length > 0) {
			...
			const choice = await new Promise(resolve => {
				rl.question(ColorConsole.getColoredStr('@xF2Select option or enter new value: @xF7'), resolve);  // Colored, no \n—readline flushes!
			});
			...
			if (choiceNum === options.length + 1 || isNaN(choiceNum)) {
				const newValue = await new Promise(resolve => {
					rl.question(ColorConsole.getColoredStr('@xF2Enter new value: @xF7'), resolve);
				});
				if (newValue.trim()) {
			...
			const value = await new Promise(resolve => {
				rl.question(ColorConsole.getColoredStr('@xF2Enter value: @xF7'), resolve);
			});
			if (value.trim()) {
